...
def streamToNoteArray(stream):
    ...
    df = pd.DataFrame({'pos': np_stream_list.T[0],
                        'dur': np_stream_list.T[1],
                        'pitch': np_stream_list.T[2]})

    df = df.sort_values(['pos','pitch'],
                        ascending=[True, False]) # sort the dataframe properly
...
@jit
def switch_range(song_array):
    """
    Switch full MIDI range to piano range
    """
    song_len = np.array(song_array).shape[0]
    ...
    for i in range(song_len):
        ...
        elif (song_array[i] in np.arange(109,128)):
            # shift down two octaves
            song_array[i] -= 44

        elif (song_array[i] in np.arange(0,21)):
            song_array[i] += 24
